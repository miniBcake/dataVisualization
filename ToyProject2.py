...
# 폰트 설정
font_path = "font/Freesentation-9Black.ttf"
...
plt.title('부침개 검색량 (년도별)', fontsize=14)

# 각 연도별 데이터를 그래프로 그리기
for i in range(len(koreanPancakeArr)):
    plt.plot(
        koreanPancakeArr[i]['period'].dt.day.astype(dtype='str'),
        koreanPancakeArr[i]['searchVolume'],
        label=f'{yearArr[i]}년'  # 범례에 연도 추가
    )
